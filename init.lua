local owner = ""
bankshop={user={},tmp={},add_storage={},max_wifi_distance=30,
...
bankshop.strpos=function(str,spl)
...
bankshop.send_mesecon=function(pos)
	if bankshop.mesecon then
		mesecon.receptor_on(pos)
		minetest.get_node_timer(pos):start(1)
	end
end

bankshop.use_offer=function(pos,player,n)
	local pressed={}
	pressed["buy" .. n]=true
	bankshop.user[player:get_player_name()]=pos
	bankshop.receive_fields(player,pressed)
	bankshop.user[player:get_player_name()]=nil
	bankshop.update(pos)
end

bankshop.get_offer=function(pos)
	if not pos or not minetest.get_node(pos) then return end
	if minetest.get_node(pos).name~="bankshop:shop" then return end
...
bankshop.receive_fields=function(player,pressed)
		local pname=player:get_player_name()
		local pos=bankshop.user[pname]
		if not pos then
			return
		elseif pressed.tsend then
			bankshop.add_storage[pname]={send=true,pos=pos}
			minetest.after(30, function(pname)
				if bankshop.add_storage[pname] then
					minetest.chat_send_player(pname, "Time expired (30s)")
					bankshop.add_storage[pname]=nil
				end
			end, pname)
			minetest.chat_send_player(pname, "Open a storage owned by you")
		return
		elseif pressed.trefill then
			bankshop.add_storage[pname]={refill=true,pos=pos}
			minetest.after(30, function(pname)
				if bankshop.add_storage[pname] then
					minetest.chat_send_player(pname, "Time expired (30s)")
					bankshop.add_storage[pname]=nil
		...
		elseif pressed.customer then
			return bankshop.showform(pos,player,true)
...
--fast checks
					if not pinv:room_for_item("main", stack) then
						...
						return
					elseif central_bank.get_balance(pname) == 0  then
						minetest.chat_send_player(pname, "Error: You dont have enough money to buy this, exchange aborted.")
						return
					else
						...
						if not check_storage then
							...
							central_bank.do_transfer(player:get_player_name(ru), owner, 10, "test")
							minetest.chat_send_player(owner, "Congratulations, "..pname.. " buyed "..rastack:get_count().." "..rastack:get_name().." from your shop!")
...
-- send to / refill from wifi storage
					if type==1 then
						local tsend=bankshop.strpos(meta:get_string("item_send"),1)
						local trefill=bankshop.strpos(meta:get_string("item_refill"),1)
						if tsend then
...
--check if its room for other items, else the shop will stuck
					...
					bankshop.send_mesecon(pos)
				end
			end
		else
			bankshop.update_info(pos)
			bankshop.update(pos,"update")
			bankshop.user[player:get_player_name()]=nil
		end
end
...
minetest.register_on_player_receive_fields(function(player, form, pressed)
	if form=="bankshop.showform" then
		bankshop.receive_fields(player,pressed)
		howmanytopay = {pay1 = pressed.pay1}
		howmanytopay = {pay2 = pressed.pay2}
		howmanytopay = {pay3 = pressed.pay3}
		howmanytopay = {pay4 = pressed.pay4}
	elseif form=="bankshop.showform2" then
		bankshop.receive_fields2(player,pressed)
	end
end)

bankshop.update_info=function(pos)
	local meta=minetest.get_meta(pos)
	local inv = meta:get_inventory()
	...
	if meta:get_int("type")==0 then
		meta:set_string("infotext","(bankshop by " .. owner ..") Stock is unlimited")
		return false
	end
	local name=""
	local count=0
	...
	for i=1,4,1 do
		...
		"(bankshop by " .. owner ..") Purchases left:\n"
...
bankshop.update=function(pos,stat)
--clear
	local spos=minetest.pos_to_string(pos)
	for _, ob in ipairs(minetest.get_objects_inside_radius(pos, 2)) do
		if ob and ob:get_luaentity() and ob:get_luaentity().bankshop and ob:get_luaentity().pos==spos then
			ob:remove()	
		end
	end
...
--update
	...
	local dp = bankshop.dir[node.param2+1]
	...
	for i=1,4,1 do
		...
		local pos2=bankshop.dpos[node.param2+1][i]
		if item~="" then
			bankshop.tmp.item=item
			bankshop.tmp.pos=spos
			local e = minetest.add_entity({x=pos.x+pos2.x,y=pos.y+pos2.y,z=pos.z+pos2.z},"bankshop:item")
...
minetest.register_entity("bankshop:item",{
	hp_max = 1,
	visual="wielditem",
	...
	on_activate = function(self, staticdata)
		if bankshop.tmp.item ~= nil then
			self.item=bankshop.tmp.item
			self.pos=bankshop.tmp.pos
			bankshop.tmp={}
		else
...
bankshop.showform=function(pos,player,re)
	local meta=minetest.get_meta(pos)
	...
	minetest.after((0.1), function(gui)
		return minetest.show_formspec(player:get_player_name(), "bankshop.showform",gui)
	end, gui)
end

minetest.register_node("bankshop:shop", {
	description = "bankshop",
	tiles = {"default_furnace_top.png^[colorize:#ffffff77^default_obsidian_glass.png"},
...
can_dig = function(pos, player)
		...
		if ((meta:get_string("owner")==player:get_player_name() or minetest.check_player_privs(player:get_player_name(), {protection_bypass=true})) and inv:is_empty("main") and inv:is_empty("pay1") and inv:is_empty("pay2") and inv:is_empty("pay3") and inv:is_empty("pay4") and inv:is_empty("give1") and inv:is_empty("give2") and inv:is_empty("give3") and inv:is_empty("give4")) or meta:get_string("owner")=="" then
			bankshop.update(pos,"clear")
			return true
		end
	end,
})

bankshop.receive_fields2=function(player,pressed)
	local pname=player:get_player_name()
	local pos=bankshop.user[pname]
	if not pos then
		return
	...
	bankshop.user[pname]=nil
end

bankshop.showform2=function(pos,player)
	local meta=minetest.get_meta(pos)
	local uname=player:get_player_name()
	...
	gui=gui
	.."tooltip[title;Used with connected bankshops]"
	...
	.."listring[current_player;main]"
	minetest.after((0.1), function(gui)
		return minetest.show_formspec(uname, "bankshop.showform2",gui)
	end, gui)

	local a=bankshop.add_storage[uname]
	if a then
		if not a.pos then return end
		if vector.distance(a.pos, pos)>bankshop.max_wifi_distance then
			minetest.chat_send_player(uname, "Too far, max distance " .. bankshop.max_wifi_distance)
		end
		local meta=minetest.get_meta(a.pos)
		local p=bankshop.strpos(pos)
		if a.send and p then
			meta:set_string("item_send",p)
		elseif a.refill and p then
			meta:set_string("item_refill",p)
		end
		minetest.chat_send_player(uname, "bankshop connected")
		bankshop.add_storage[uname]=nil
	end
end

minetest.register_node("bankshop:wifistorage", {
